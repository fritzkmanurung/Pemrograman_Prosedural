...
int main(int _argc, char **_argv)
{
    ...
    struct dorm_t left_dorm = create_dorm("left", 999, GENDER_MALE);
    ...
    while(fgets(line, sizeof(line), stdin)) {
        ...
        else if(strcmp(cmd, "student-leave") == 0) {
            char *id = strtok(NULL, "#");
            if(id) {
                int s_idx = -1;
                for(int i = 0; i < student_count; i++) {
                ...
                if(s_idx >= 0 && students[s_idx].dorm != NULL && students[s_idx].dorm != &left_dorm) {
                    students[s_idx].dorm->residents_num--;
                    students[s_idx].dorm = &left_dorm;
                }
            }
        }
    }
