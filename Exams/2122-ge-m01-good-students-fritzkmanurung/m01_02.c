...
int main(int _argc, char **_argv) {
    ...
    while(fgets(line, sizeof(line), stdin)) {
        ...
        else if(strcmp(cmd, "student-show-best") == 0) {
            if(count > 0) {
                int best_idx = 0;
                float best_gpa = atof(students[0].gpa);
                for(int i = 1; i < count; i++) {
                    float current_gpa = atof(students[i].gpa);
                    if(current_gpa > best_gpa) {
                        best_gpa = current_gpa;
                        best_idx = i;
                    }
                }
                printf("%s|%s|%s\n", students[best_idx].id, students[best_idx].name, students[best_idx].gpa);
            }
        }
    }
